...
class InMemoryRepositoryTests: XCTestCase {
  ...
  func test_list_ShouldEmitEntities() -> Void {
  ...
  func test_listBy_ShouldEmitEntitiesByListOfIds() -> Void {
    let byIds = [90, 91]
    ...
    collect(stream$: repository.listBy(keyPath: \DataAccessEntity.otherId, ids: byIds), collect: 1, cancellables: &cancellables)
      .sink(receiveCompletion: { _ in }) {
