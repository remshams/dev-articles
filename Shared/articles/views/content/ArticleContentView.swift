...
private struct Header: View {
  ...
  var body: some View {
    VStack {
      Text(article.title).font(.largeTitle).foregroundColor(.black).padding(.bottom, 16).padding(.leading, 8)
...
private struct NoCoverImage: View {
  ...
  struct ArticleContent_Previews: PreviewProvider {
    static var previews: some View {
      ArticleContentView(article: articleForPreview)
    }
  }

  // swiftlint:disable all
  private let exampleContent = """
